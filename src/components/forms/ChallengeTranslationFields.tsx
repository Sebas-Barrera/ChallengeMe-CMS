...
interface ChallengeTranslationFieldsProps {
  selectedLanguages: string[];
  translations: Record<string, ChallengeTranslation>;
  onChange: (languageCode: string, content: string) => void;
  onAutoTranslate?: (targetLanguage: string) => void;
  errors?: Record<string, string>;
}

export default function ChallengeTranslationFields({
  selectedLanguages,
  translations,
  onChange,
  onAutoTranslate,
  errors = {},
}: ChallengeTranslationFieldsProps) {
    ...
    return (
      <div className="bg-bg-secondary/50 border border-border rounded-xl p-6 text-center">
        ...
        <p className="text-text-secondary">
          Selecciona al menos un idioma para agregar el contenido del reto
        </p>
      </div>
    );
  }
        ...
        return (
          ...
          >
            ...
            <Textarea
              ...
              placeholder={`Escribe el reto en ${language.nativeName}...`}
              value={translation.content}
              onChange={(e) => onChange(languageCode, e.target.value)}
              error={errors[languageCode]}
              rows={4}
              required
              helperText="El texto del desafío que verán los usuarios"
            />
          </div>
        );
      })}
